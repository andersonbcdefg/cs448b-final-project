...
const fetchData = async (path) => {
	...
	const data = res.map(entry => {
		return {
			...
			comparison: Number(entry[Object.keys(entry)[2]]),
			us: Number(entry[Object.keys(entry)[1]])
		}
	});
...
const makeScales = (data, params) => {
	...
	if (params.includeZero) {
		const yScale = d3.scaleLinear()
			.domain([0, d3.max(data, d => d.us) * 1.1])
			.range([params.height + params.marginY, params.marginY])

		return [xScale, yScale];
	}
	const yScale = d3.scaleLinear()
		.domain([d3.min(data, d => d.us) * 0.75, d3.max(data, d => d.us) * 1.1])
		.range([params.height + params.marginY, params.marginY])
